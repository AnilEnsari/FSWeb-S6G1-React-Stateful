...
for(let i = 0;i<programmer.length;i++){if (programmer[i].id===programmerId)
  developer= programmer[i].isim}
  return developer
  };
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (programmerId)?'gold':'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        ...
        programmer.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setProgrammerId(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={stil}>
        {
          ...
          (programmerId) ? `🎉 Hadi ${oneCikaninIsmi()}'ı kutlayalım! 🥳`
